<template>
  <div class="waterfall" ref="containerRef" @scroll="handleScroll">
    <div class="waterfall-list" ref="listRefs">
      <div
      ...
      >
        <slot name="item" :item="item" :index="index" :image-height="state.cardPos[index].imageHeight"></slot>
      </div>
    </div>
  </div>
</template>
...
const listRefs = ref<HTMLDivElement | null>(null);
...
const handleResize = debounce(() => {
  ...
  state.cardWidth = (containerWidth - props.gap * props.column) / props.column;
...
const computedRealDomPos = (list: ICardItem[]) => {
  const children = listRefs.value!.children;
...
const init = () => {
  if (containerRef.value) {
    const containerWidth = containerRef.value.clientWidth;
    state.cardWidth = (containerWidth - props.gap * (props.column - 1)) / props.column; //计算卡片宽度
